...
auto parseValue(std::vector<token::Token>& tokens, int& pc, const bool composite = true,
                const bool fncheck = true) -> std::shared_ptr<Value>;
...
auto check_value(const token::Token tkn) -> bool {
  using namespace token;
  return tkn.type == tkn_type::Num || tkn.type == tkn_type::String || tkn.type == tkn_type::Iden ||
...
auto parseMath(std::vector<token::Token>& tokens, int& pc) -> std::shared_ptr<Math> {
  ...
  if (pc + 2 < tokens.size() &&
      tokens[pc + 1].type != token::tkn_type::End) {  // safety for larger requirements
...
auto parseValue(std::vector<token::Token>& tokens, int& pc, const bool composite,
                const bool fncheck) -> std::shared_ptr<Value> {
  using namespace token;
  ...
  if (tokens[pc].type == token::tkn_type::Scope_start) {
    ...
    if (auto value_asmt = std::get_if<std::shared_ptr<Value>>(&loop_node->asmt->val)) {
      if (auto fncall = std::dynamic_pointer_cast<FnCall>(*value_asmt)) {
        if (auto scope_arg =
              std::get_if<std::shared_ptr<Scope>>(&fncall->args[fncall->args.size() - 1])) {
